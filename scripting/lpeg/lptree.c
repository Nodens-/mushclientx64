...
*/
static size_t addtoktable(lua_State* L, int idx)
{
...
*/
static size_t ktablelen(lua_State* L, int idx)
{
...
{
	int i;
	int n1 = (int) ktablelen(L, idx1);
	int n2 = (int) ktablelen(L, idx2);
	if (n1 + n2 > USHRT_MAX)
		luaL_error(L, "too many Lua values in pattern");
...
static void joinktables(lua_State* L, int p1, TTree* t2, int p2)
{
	...
	n1 = (int) ktablelen(L, -2);
	n2 = (int) ktablelen(L, -1);
...
static size_t getsize(lua_State* L, int idx)
{
...
static int lp_constcapture(lua_State* L)
{
	...
	{  /* create a group capture with all values */
		...
		for (i = 1; i <= n - 1; i++)
		{
			...
			sib1(tree)->key = (unsigned short) addtoktable(L, i);
			tree = sib2(tree);
		}
		auxemptycap(tree, Cconst);
		tree->key = (unsigned short) addtoktable(L, i);
	}
	return 1;
}
...
static int collectrules(lua_State* L, int arg, int* totalsize)
{
	...
	size = 2 + (int) getsize(L, postab + 2);  /* TGrammar + TRule + rule */
	...
	while (lua_next(L, arg) != 0)
	{
		...
		size += 1 + (int) getsize(L, -1);  /* update size */
...
static void initialrulename(lua_State* L, TTree* grammar, int frule)
{
	...
	{  /* initial rule is not referenced? */
		size_t n = lua_rawlen(L, -1) + 1;  /* index for name */
		lua_pushvalue(L, frule);  /* rule's name */
		lua_rawseti(L, -2, (int) n);  /* ktable was on the top of the stack */
		sib1(grammar)->key = (int) n;
	}
}
