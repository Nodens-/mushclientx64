...
static int xmlread(lua_State* L)
{
	...
	try
	{
		...
		CMemFile f((unsigned char*) xml, (int) xmlLength);
	...
	for (lua_pushnil(L); lua_next(L, table) != 0; lua_pop(L, 1))
	{
		...
		if (lua_type(L, -2) == LUA_TSTRING)
		{
			...
			if (bHaveDefault &&
				...
				msg.m_iDefault = (int) msg.m_data.size();
		}
		else
	  ...
	  // remember default positions
		if (bHaveDefaults)
		{
			...
			if (!(lua_isnil(L, -1) ||
				...
				msg.m_iDefaults.insert((int) msg.m_data.size());
...
static int utf8valid(lua_State* L)
{
	...
	int erroroffset;
	int iBad = _pcre_valid_utf((unsigned char*) data, (int) length, &erroroffset);
...
static int utf8sub(lua_State* L)
{
	...
	int erroroffset;
	int iBad = _pcre_valid_utf((unsigned char*) s, (int) length, &erroroffset);
...
static int getfontfamilies(lua_State* L)
{
	...
	EnumFontFamiliesEx(dc.m_hDC,           // handle to device context
		...
		(LONG_PTR) L,           // application-supplied data
...
static int shell_execute(lua_State* L)
{
	...
	int result = (INT_PTR) ShellExecute(NULL,
		operation,
		filename,
