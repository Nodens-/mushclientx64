...
void CMiniWindow::FontList (VARIANT & vaResult)
  ...
  if (!m_Fonts.empty ())    // cannot create empty dimension
    {
    sa.CreateOneDim (VT_VARIANT, (DWORD)m_Fonts.size ());
...
static long CalculateUTF8length (LPCTSTR Text, size_t length)
  ...
  int erroroffset;
  int iBad = _pcre_valid_utf ((unsigned char  *) Text, (int)length, &erroroffset);
  if (iBad > 0)
    return -1;
...
long CMiniWindow::Text (LPCTSTR FontId,  // which previously-created font
  ...
  if (Unicode)
    ...
    pdc->ExtTextOut (Left, Top, ETO_CLIPPED, CRect (Left, Top, FixRight (Right), FixBottom (Bottom)),
                  Text, (UINT)length, NULL);
    textsize = pdc->GetTextExtent (Text, (int)length);
...
void CMiniWindow::ImageList(VARIANT & vaResult)
  ...
  if (!m_Images.empty ())    // cannot create empty dimension
    {
    sa.CreateOneDim (VT_VARIANT, (DWORD)m_Images.size ());
...
void CMiniWindow::HotspotList(VARIANT & vaResult)
  ...
  if (!m_Hotspots.empty ())    // cannot create empty dimension
    {
    sa.CreateOneDim (VT_VARIANT, (DWORD)m_Hotspots.size ());
...
CPoint menupoint (Left, Top);
  ...
  for (vector<string>::const_iterator i = v.begin (); i != v.end (); i++)
    ...
    if (*i == "-" || *i == "")
      ...
      if (pNestedMenu)
        {
        vPopup.back ()->AppendMenu (MF_POPUP | MF_ENABLED, (UINT_PTR ) pNestedMenu->m_hMenu, strItem.Mid (1));
