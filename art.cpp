...
static void readfontchar(CStdioFile& file, inchr theord, char* line, int maxlen)
{
	...
	fclsave = fcharlist;
	fcharlist = reinterpret_cast<fcharnode*> (myalloc(sizeof(fcharnode)));
	fcharlist->ord = theord;
	fcharlist->thechar = reinterpret_cast<char**> (myalloc(sizeof(char*) * charheight));
	...
	for (row = 0; row < charheight; row++)
	{
		...
		line[k + 1] = '\0';
		fcharlist->thechar[row] = static_cast<char*>(myalloc(sizeof(char) * (k + 2)));
...
static void readfont(CString strName)
{
	...
	fileline = static_cast<char*> (myalloc(sizeof(char) * (MAXFIRSTLINELEN + 1)));
	...
	fileline = static_cast<char*> (myalloc(sizeof(char) * (maxlen + 1)));
	...
	for (row = 0; row < charheight; row++)
	{
		fcharlist->thechar[row] = static_cast<char*> (myalloc(sizeof(char)));
...
static void linealloc()
{
	...
	outputline = reinterpret_cast<char**> (myalloc(sizeof(char*) * charheight));
	for (row = 0; row < charheight; row++)
	{
		outputline[row] = myalloc(sizeof(char) * (outlinelenlimit + 1));
	}
	inchrlinelenlimit = outputwidth * 4 + 100;
	inchrline = reinterpret_cast<inchr*> (myalloc(sizeof(inchr) * (inchrlinelenlimit + 1)));
	clearline();
}
...
static int addchar(inchr c)
{
	...
	templine = myalloc(sizeof(char) * (outlinelenlimit + 1));
	for (row = 0; row < charheight; row++)
	{
		if (right2left)
		{
...
void CTextView::OnEditAsciiart()
{
	...
	if (dlg.m_iLayout != static_cast<int> (App.m_iAsciiArtLayout))
